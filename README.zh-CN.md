...
AIGC CANVAS 是一个面向完整 AI 短剧生产闭环的 Harness Engineering 桌面工作台。用户输入剧本后，Agent 会为每个角色先生成唯一四区身份底图，再通过图生图派生不同场景/服装版本，同时生成无人俯视全景场景图，组织多模态参考并调用 ComfyUI 生成视频；需要时可按用户的具体问题分析实际视频。
...
- **角色与场景一致性资产**：每个角色先生成唯一 16:9 四区角色设计图（正面无头全身、背面含后脑不露脸全身、右上中性 3/4 特写、右下微笑 3/4 特写），再以底图为单一参考通过图生图派生场景/服装版本。场景资产使用无人高机位俯视全景图。
...
- **RTX 视频放大**：视频放大节点支持 2× / 3× / 4× 和 LOW / MEDIUM / HIGH / ULTRA 质量档位，输出帧率自动跟随源视频。
...
3. 资产 Agent 读取原剧本，调用人物/场景 Skill：人物 Prompt 经 `image-prompt-skill` 写出四区设计图，先生成每个角色唯一身份底图和无人俯视场景图，再由底图图生图派生服装/状态版本；保留分阶段生成确认。核实画布资产后输出 `资产报告.md`，列出节点 ID、角色/场景版本、素材路径和可用状态。
