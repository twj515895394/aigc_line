...
6. **生成状态**：`generationStatus`（idle/generating/error）是只读轮询字段；异步动作完成后必须复位，加载快照时会把残留的 generating 重置为 idle。`error` 只表示该节点失败：停止该节点及其下游，把节点 id 和 `generationError` 告诉用户；不要打断仍在 generating 的无关并行节点，重做只针对失败节点。画布在 `generate` 时若连入的 image/video/audio 失败、仍在生成或没有 `sourcePath`，拒绝本节点生成。失败节点会由生成动作自动重试最多 3 次，再切设置里的备用模型按同样规则重试。
