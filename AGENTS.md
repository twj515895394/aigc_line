...
| `src/components/CanvasBackground.tsx` | 随世界坐标移动的 CSS 点阵背景，每动画帧直接更新位置，避免 SVG pattern 逐输入事件布局 |
...
- **内置 Skill**：`aigc-canvas` 本地 Plugin 随应用打包，Claude 由 SDK plugins 加载；项目级 `.claude/skills` 仍可自动发现。Codex 扫描项目 `.agents/skills`、用户 `.agents/skills` 与 `.codex/skills`，以及同一内置 Skill 目录；系统指令列出路径，显式斜杠调用会要求读取对应 SKILL.md。应用不会向项目复制或覆盖 Skill。
  - `voiceover-to-video`：按旁白音频与 SRT 时间轴生成画面；图片提示词统一使用中文，视频提示词按一个节点内多个带时间段的子分镜描述统一风格、运镜、转场与音效；禁止生成 BGM，但允许不遮盖旁白的环境音和拟音。开始前五项参数一次性确认；图片与视频生成前分别取得用户明确确认；生成视频后只核对节点状态和产物路径，逐段变速对齐后交给 `jianying-draft` 创建剪映草稿。
  - `script-to-drama-video`：统一承接短剧创作、分镜规划、现有 image → video 链修改和连续性修复。主 Agent 串行调度三个真实子 Agent：资产 Agent 读取原剧本、生成/复用人物场景道具并输出 `资产报告.md`；分镜师 Agent 读取原剧本和资产报告，拆分 5/10/15 秒片段及内部 Shot、创建视频节点/引用/连线、逐片调用 H3 Skill 并输出 `分镜交接.md`；独立检查 Agent 只读实际画布，核对人物引用、分段连续性、必要补镜/描述和 prompt，输出每轮检查报告。未通过则优化后复查，直到全部待生成片段通过且获得用户生成授权才生成；阻塞不得跳过，通过后内容变更须复查。报告保存在项目 `generated/drama-reports/<本次任务标识>/`，交接规范在 `references/agent-handoffs.md`，导演方法在 `references/directing-and-continuity.md`。新任务画幅/风格/投放/分段等无依赖参数一次性确认，参考图分阶段授权和视频生成授权仍分阶段；人物/场景/H3 专项 Skill 由对应角色调用；生成后只核对节点状态和产物路径，不自动执行成片视频审核。
  - `character-reference-generation`：每个角色先生成唯一身份底图，再以底图为单一参考通过图生图派生不同场景/服装/妆造/状态版本。角色设计图版式与 Prompt 必须调用 `image-prompt-skill` 的角色资产规范（16:9 三大版面四功能区：正面无头全身、背面含后脑不露脸全身、右上中性 3/4 特写、右下微笑 3/4 特写）；禁止旧四联和 A-pose/T-pose。3D/半写实只借用风格档位与材质渲染用语，不得沿用旧四联排版。
  ...
  - `h3-prompt-writing`：MiniMax H3 官方提示词写作 Skill，负责 T2VA / I2VA / FL2VA / L2VA / Ref2VA 的最终提示词格式、引用标签、时间戳、对白和声音字段。导演台预演/关键帧是可选参考层：有则按实际职责活写进官方英文字段（预演常见为镜头/空间/宏观动作，关键帧常见为构图锚点），不要另开中文「导演台参考约束」区块，也不要套死模板把未指定的维度写死；无导演台素材时按普通 H3 写。`script-to-drama-video` 提供完整逐片段导演包、片段内 Shot 时间线及真实引用数组顺序，并在 Ref2VA 视频生成或重做时调用它，不自行复制或猜测官方格式。
  - `image-prompt-skill`：把文字或参考图写成可投喂生图模型的完整中文 Prompt，覆盖成片、图生图、局部编辑、反推、角色/场景/分镜资产和视频参考帧。短剧角色设计图以本 Skill 的四区规范为版式真源；`character-reference-generation` 只负责节点、连线和授权门。场景俯视仍由 `environment-reference-generation` 锁定。
  - `video-prompt-skill`：把文字、图片、视频或音频写成可投喂视频模型的导演 Prompt 与时间轴。缺时长时按 3–15 秒生产，本仓库节点有 `duration` 时以节点为准、未指定默认 10 秒。H3 视频仍须再经 `h3-prompt-writing` 转官方格式；Grok / Seedance 等非 H3 以本 Skill 终稿写入 `video.prompt`。正式模型适配页目前为 Generic / Seedance 2.0 / LTX-2.3。画布上 `generated/director-videos/` 预演视频与 `generated/director-stills/` 关键帧按导演台参考层绑定：预演继承镜头/空间/宏观动作且默认不继承声音与代理外观，关键帧只锁定静态锚点；两者同时存在时不得平均融合。
  - `inverse-video-prompt`：把参考视频、片段或连续帧反推成可生成的视频提示词（镜头时间线、表演、运镜、光影、连续性卡与推荐声音）。默认忠实复刻；structural / style-only 才只学镜头不抄人物。有完整视频时必须先调用 `AnalyzeVideo`（设置页已配置的 Qwen 或 Gemini 音画分析模型），本 Skill 只编译 Prompt，不替代质量审核、不改打未配置模型。需要参考视频转绘时：分析 → 反推 Prompt → `video-prompt-skill` 按保真模式改写 → 按 `workflowId` 写入 video 节点（H3 再经 `h3-prompt-writing`）。
  - `codex-image-generation`：仅 Codex，且仅用户显式 `/aigc-canvas:codex-image-generation` 时执行，禁止因生图类自然语言自行选用。用内置 `image_gen` 生成或编辑位图，拷到项目 `generated/images/`，需要时把相对路径写入 image 节点 `sourcePath`。禁止改打画布 `generate` / ComfyUI；没有 `image_gen` 工具时停止。不替代角色底图、场景俯视或短剧生产 Skill。
...
- **3D 导演台 Agent 状态条**：导演台标题栏下方常驻一条状态：空闲显示「Agent 空闲」；回合进行中显示当前动作（添加元素、自检、拍摄关键帧、导出预演及帧进度）；失败显示最近 `generationError`。状态来自 Agent 回合、正在执行的工具、本节点 `generationStatus` 和导演台导出请求，不另做轮询通道。
...
- **节点类型**（`CanvasNodeKind`）：
  - `image` 图片：ComfyUI 的 Krea 2 Turbo、Z-Image Turbo 当前仅文生图；Nano Banana 2 / Pro 支持最多 14 张有序参考图，Doubao-Seedream-5.0-pro / lite、Gemini 反代与 GPT/Grok 反代支持最多 10 张有序参考图；画幅支持 16:9 / 9:16 / 1:1 / 4:3，全部模型使用 2K 输出，ComfyUI 图片工作流直接保存 VAE 解码结果且不经过 RTX 放大。工作流选择器与设置默认项必须标明 ComfyUI 或在线，禁止把 `gpt-grok-image:` / `gemini-proxy-image:` 显示成 ComfyUI 文生图
  ...
  - `video` 视频：MiniMax H3 一采 `minimax-h3-easy` 是默认工作流，同一套图支持文生、首尾帧和图/视频/音频全模态参考；有参考轨时 `mode=reference` 并加载 Ref2V Turbo LoRA，无参考轨时 `mode=image` 并加载 FL2V Turbo LoRA，默认 10 步。二采 `minimax-h3-easy-2pass` 需手动选择：335 注入与一采相同，但双采样（总步数 9 / 一采步数 6），不按 mode 替换 265/280 的双路 LoRA，产物从节点 `291` 保存，质量更高、更慢。旧的 `minimax-h3-t2v-flf2v` / `minimax-h3-r2v` / `minimax-h3-r2v-turbo` 仍可用。视频画幅含 16:9 / 9:16 / 1:1 / 4:3 / 3:4。H3 参考视频通过 `GetVideoComponents` 同时把画面接入 `ref_videos`、内嵌音轨接入相同下标的 `ref_video_audios`（一采/二采则把整段 VIDEO 接到 `video_N`）；画布仍可显式提取音轨生成独立 audio 节点并放入单独音频参考轨；也支持火山方舟 Agent Plan Doubao Seedance 2.0 文生视频与全模态参考，提示词用“图片 n / 视频 n / 音频 n”引用素材，默认 720p 并生成同步音频
  - `audio` 音频：导入本地音频并预览
  - `upscale` 视频放大：RTX Video Super Resolution，连入视频节点作为输入（多输入可点选，`inputNodeId`），倍数 2x/3x/4x，质量 LOW/MEDIUM/HIGH/ULTRA，帧率经 VHS_VideoInfo 自动跟随源视频
...
- **图片/视频生成集成**：ComfyUI 工作流模板在 `resources/comfyui-workflows/`，`comfyui.service.ts` 注入参数 → 排队 → 轮询 history → 下载结果；默认 ComfyUI 文生图为 `krea2-turbo-t2i`，使用 `krea2\krea2_turbo_int8_convrot.safetensors` 和 8 步 Euler/simple 采样，Krea 2 Turbo 与 Z-Image Turbo 均直接以标准 2K 尺寸生成并保存，图片工作流不包含 RTX 放大节点；旧 Flux2 Klein 文生图/图生图工作流已移除且旧默认设置自动迁移到 Krea 2 Turbo。`minimax-h3-easy` 是 FeiHou 一采默认项：文生/图生走 FL2V Turbo LoRA，全模态参考走 Ref2V Turbo LoRA，默认 10 步。`minimax-h3-easy-2pass` 是手动选择的二采：335 动态项与一采相同，双采样总步数 9 / 一采步数 6，不替换双路 LoRA，SaveVideo 为节点 `291`。旧 `minimax-h3-r2v` / `minimax-h3-r2v-turbo` / `minimax-h3-t2v-flf2v` 仍保留。MiniMax H3 视频默认 0.98MP：16:9 / 9:16 / 4:3 / 3:4 / 1:1 分别为 1344×768 / 768×1344 / 1152×864 / 864×1152 / 992×992。火山方舟 `seedance-2.0` 工作流使用模型 `doubao-seedance-2-0-260128`，通过 `/contents/generations/tasks` 异步提交与轮询，支持 4–15 秒文生视频和最多 9 图 + 3 视频 + 3 音频的项目内参考素材，默认 720p、同步音频、无水印，完成后下载到 `generated/videos/`。ComfyUI/Google 图片使用标准 2K 映射：16:9 为 2048×1152、9:16 为 1152×2048、4:3 为 2048×1536、1:1 为 2048×2048；Seedream 使用官方 2K 参考尺寸 2816×1584、1584×2816、2368×1776、2048×2048。Google Gemini 图片 API 由 `google-image.service.ts` 调用 Nano Banana 2 / Pro；火山方舟图片 API 由 `seedream-image.service.ts` 调用 Doubao-Seedream-5.0-pro / lite；两者固定生成 2K 图片并支持项目内参考图。
- **设置页**：分两个可折叠区。上区「模型与服务」放 ComfyUI、Qwen 审查、Gemini 反代、方舟、GPT/Grok 反代；下区「默认生成模型」分别选默认图片/视频工作流，并可各标最多两个带备注的备用模型。新建节点和缺省 generate 写入默认项；默认因网络/额度/503/超时失败时生成动作自动切备用1、备用2并改写节点 workflowId，手选模型不自动改。不要落到列表第一项（例如豆包 Seedream）。Gemini 与 GPT/Grok 的「获取模型」只刷新卡片列表，不写 settings；落盘仍用顶部「保存配置」。Qwen/Gemini/GPT-Grok Key 使用 safeStorage；方舟 Key 明文保存在本机 `settings.json`。音视频分析后端二选一（默认 Qwen）。画布图片/视频工作流列表会追加设置里勾选的反代模型（`gemini-proxy-image:` / `gpt-grok-image:` / `gpt-grok-video:`），ComfyUI 与方舟项始终保留。设置勾选与默认模型列表必须标「在线」或「ComfyUI」。反代生图按模型 ID 子串路由（gemini > grok > gpt 兜底）分成 GPT / Grok / Gemini 三套 payload，不要共用一套：GPT 文生图（无参考图）向 `/images/generations` 发 JSON；GPT 图生图（有参考图）按官方 OpenAI 契约向 `/images/edits` 发 multipart/form-data，表单字段 `model/prompt/size` 加每张参考图一个名为 `image[]` 的文件部分（保留原文件名），禁止在 JSON body 里塞 `images` / `image_url` 数组——CLIProxyAPI 网关会在 `/images/generations` 上静默丢弃 JSON `images` 字段（参考图到不了模型）；Grok 与 Gemini 都向 `/images/generations` 发 JSON，参考图转 data URI 放进 `image` 数组，禁止套用 GPT 的 multipart 约定。参考图按项目根 realpath 边界检查，白名单 PNG/JPEG/WebP，每节点最多 10 张、单张 ≤20MB（GPT/Grok 与 Gemini 反代共用 `loadOpenAiReferenceImages`）。共享构造器在 `src/shared/gpt-image-call.ts`（GPT 族 json/multipart 分支）与 `src/shared/reverse-proxy-image.ts`（三族路由），请求组装在 `electron/main/services/reverse-proxy-image.ts`。
...
- **聊天可靠性与分段展示**：同一 messageId 的完整正文推送按 ID 更新；Claude 助手气泡用 SDK `message.id`（缺则 uuid）upsert，同一轮重复 assistant 帧更新原消息，不新造 `agent-${Date.now()}` 导致双气泡；工具事件按工具 ID 合并，历史请求使用加载代次与项目选择代次拒绝 A→B→A 的旧响应，并合并加载期间的新事件。读取历史只把没有实际执行者的遗留 running 工具标为中断。发送等待主进程确认，失败保留正文、文件附件及节点/Artifact 引用，成功也不能清掉期间新输入的草稿。界面初始显示最近 100 条，按 100 条加载更早消息并维持阅读位置；这属于前端分段展示，历史 IPC 仍返回完整消息列表，不是服务端分页。
...
- **放大视图连线绘制**：`CanvasEdge` 保留 React Flow 的图结构、选择/删除与命中路径，仅将三次贝塞尔通过 De Casteljau 拆为视区内的精确子曲线，不能用折线近似或按“端点都离屏”直接隐藏仍穿过屏幕的连线。裁剪窗口按 512 世界单位分块并留 256 屏幕像素缓冲，平移逐像素时不必重算全部路径；箭头仅绘制在含真实源/目标端点的单子路径上，禁止在裁剪断点出现假箭头。未知路径格式回退原路径，不得丢边；绘制裁剪不写回画布快照。小地图通过独立合成层及 layout/paint containment 隔离主画布的重绘。
...
| 新增 IPC 接口 | `src/shared/ipc.types.ts` + `src/shared/ipc.channels.ts` + `electron/main/ipc/*.handlers.ts` + `electron/preload/index.ts` 四处一起改。聊天产物打开目录走 `project.showItemInFolder`，主进程校验必须落在已登记项目目录内；文件缺失时打开所在目录。 |
| 本地媒体预览协议 | `electron/main/index.ts` 的 `protocol.handle('workspace' | 'local-file')` |


## 二开与主分支合入

当前仓库在主分支之上拉开发分支做二开。后续会把主分支更新合入本分支：**不逃避冲突，但必须把冲突面压到接缝上**，让少量冲突可预期、可人工把握。

- **单一职责**：一个模块只做一件事（一种协议、一个提供方、一种设置卡片、一种生成后端）。不要把 Gemini 反代、GPT/Grok 反代、Qwen、ComfyUI、方舟揉进同一个巨型服务或同一大段 UI。
- **主干少改、二开外置**：主分支已有文件（尤其 `CanvasArea.tsx`、`comfyui.service.ts`、`SettingsPage.tsx`、IPC handler 聚合点）只加**最短路由/注册**。新协议、新模型目录、新 Key、新轮询实现放到新文件。禁止在主干函数里堆第二套完整生成逻辑。
- **接缝稳定**：用窄接口对接主干（例如「是否某类 workflowId」「生成图片/视频」「列出工作流」）。接口稳定后，主分支改内部实现时冲突应落在一行 `if` / 一张注册表，而不是整段业务。
- **允许的冲突**：注册表多一行、设置卡片多一块、provider 联合类型多一个字面量。不允许的冲突：在主干大函数中段插入几十行反代细节，导致每次合主都要手工重演。
- **合入态度**：主分支有更新就合。冲突出现时按「保留主干结构，把二开代码移回自己的模块」解决，不要为了消冲突把二开逻辑继续摊进主干文件。
...
6. **生成状态**：`generationStatus`（idle/generating/error）是只读轮询字段；异步动作完成后必须复位，加载快照时会把残留的 generating 重置为 idle。`generating` 含同一模型最多 3 次重试以及切备用1/备用2，期间不要写 generationError，Agent 必须继续轮询、禁止自行改 workflowId。`error` 只表示该节点最终失败：停止该节点及其下游，把节点 id 和 `generationError` 告诉用户；不要打断仍在 generating 的无关并行节点，重做只针对失败节点。画布在 `generate` 时若连入的 image/video/audio 失败、仍在生成或没有 `sourcePath`，拒绝本节点生成。除提示词缺失、敏感内容、未知提交等硬停止外，失败都会自动重试再切设置里的备用模型。
...
14. **剧本深化、导演方法、资产委派与片段层级**：`script-to-drama-video` 是唯一通用短剧分镜 Skill；`storyboard-production` 已删除。新任务在画幅、风格、投放形态和时长拆分未确认前不得创建节点或默认 `9:16`/`16:9`。默认保留核心人物关系、事实、冲突、因果和结局方向，允许为视听表达补足动作、反应、潜台词、必要对白/旁白和声画衔接；改变核心动机、关键事件或结局必须先确认。先按 `references/directing-and-continuity.md` 建立节拍、blocking、切镜理由和连续性账本，再拆成可独立生成的 5/10/15 秒片段；每个片段直接创建一个 video 节点，片段内 Shot 使用连续时间范围写入导演包和 prompt，不创建 Canvas 节点。人物图必须调用 `character-reference-generation`，其 Prompt 必须经 `image-prompt-skill` 角色资产写出默认四区设计图：每个 `characterId` 先生成唯一身份底图，审核合格并取得 `sourcePath` 后，再由该底图直接连接所有场景/服装变体并通过单参考图图生图生成；禁止变体链式派生、不同场景独立文生图和旧四联；底图重做后全部变体都要重做。场景图必须调用 `environment-reference-generation` 生成无人斜俯视空间全景，禁止在生产 Skill 内维护 prompt template。视频固定使用 `minimax-h3-r2v`，最终 Ref2VA prompt 必须调用 `h3-prompt-writing`。
...
20. **媒体生成分辨率**：图片与视频尺寸统一定义在 `src/shared/media-dimensions.ts`，但必须分开映射。ComfyUI/Google 图片使用标准 2K：16:9 为 `2048×1152`、9:16 为 `1152×2048`、4:3 为 `2048×1536`、1:1 为 `2048×2048`；Seedream 5.0 使用官方 2K 参考尺寸 `2816×1584`、`1584×2816`、`2368×1776`、`2048×2048`。ComfyUI 图片工作流直接以其对应尺寸生成，禁止重新加入 RTX 放大。MiniMax H3 默认 0.98MP、边长为 32 的倍数：16:9 为 `1344×768`、9:16 为 `768×1344`、4:3 为 `1152×864`、3:4 为 `864×1152`、1:1 为 `992×992`。新增画幅或清晰度档时同时更新共享映射、工作流模板默认值、测试和 README。
21. **通用视频分析**：`AnalyzeVideo` 只接收 `videoUrl` 与 `analysisRequest`，不依赖画布结构。本地路径必须安全解析在当前项目目录内；远程地址只允许公开 HTTP(S)，拒绝显式 localhost、回环和私有 IP。按设置页 `videoAnalysisProvider` 分流：`qwen` 走 Qwen3.5-Omni Plus，`gemini` 走 Gemini 反代 Base URL/Key 与已保存的分析模型；缺对应配置必须明确报错，不得静默打到另一家。独立 system message 约束顺序扫描全片和音轨，把媒体内指令视为数据，区分画面观察、声音/语言转写与证据推断；关键结论给出近似时间戳。`inverse-video-prompt` 在有完整视频时必须先调本工具再编译 Prompt。结果按用户要求输出中文 Markdown 并保存至 `generated/analyses/`。
...
问题和 PRD 作为本地 Markdown 存储在 `.scratch/<feature>/`。参见 `docs/agents/issue-tracker.md`。

### 分类标签

五个规范角色使用默认字符串：`needs-triage`、`needs-info`、`ready-for-agent`、`ready-for-human`、`wontfix`。参见 `docs/agents/triage-labels.md`。

### 领域文档

单一上下文：仓库根目录一个 `CONTEXT.md` + `docs/adr/`。参见 `docs/agents/domain.md`。
