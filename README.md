...
Agent 可以用 `PushArtifact` 把项目内的图片、Markdown 报告、UTF-8 文本/代码和 HTML 页面显示为聊天中的轻量产物卡片，点击后弹窗放大查看，也可添加到对话。内容只在弹窗打开时加载，关闭后卸载。相同文件再次推送后，卡片打开最新内容；重开项目后仍保留。推送产物不会创建或更新画布节点；如需把图片放到画布，Agent 使用 `CreateCanvasNodes` 创建 image 节点，或用 `UpdateCanvasNodes` 更新已有节点的 `sourcePath`。旧画布文档在备份和保存聊天记录后迁移。HTML 资源路径相对源文件所在目录解析；文本上限 1 MB，图片上限 20 MB。PDF、Office、压缩包暂不支持；视频和音频使用现有媒体节点。
